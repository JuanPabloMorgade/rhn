...
export async function formatEmailHtmlClient(htmlRaw: string): Promise<string> {
  ...
  container.querySelectorAll('mark').forEach(mark => {
    ...
    const computedColor = window.getComputedStyle(mark).getPropertyValue('color');
    const span = document.createElement('span');
    span.textContent = text;
    if (bg) span.style.backgroundColor = bg;
    if (computedColor) span.style.color = computedColor;
    const align = mark.getAttribute('data-align');
    if (align) span.style.textAlign = align as any;
